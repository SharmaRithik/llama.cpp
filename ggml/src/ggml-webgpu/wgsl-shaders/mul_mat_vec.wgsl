...
#define THREADS_PER_BLOCK 16
    ...
    // Each thread covers half of one 32-weight sub-block: 2 slots * 8 grid entries = 16 weights.
    let sub_blk = tid / 2u;
    let half    = tid % 2u;
    let slot0   = half * 2u;
    ...
    for (var block = block_group; block < num_blocks; block += num_block_groups) {
        ...
        for (var row = 0u; row < OUTPUTS_PER_WG; row++) {
            ...
            if (output_row < params.m) {
                ...
                // Reconstruct d f16 from top nibbles of 4 u16 scales at bytes 48..55.
                ...
                // Pick which 16-bit scale word applies to this sub_blk.
...
#define THREADS_PER_BLOCK 16
    ...
    // IQ4_XS layout: one 32-weight sub-block is 16 bytes. Byte b stores weight b
    // in its low nibble and weight b+16 in its high nibble, so the two 16-weight
    // halves read the same 16 bytes but different nibbles.
    let sub_blk = tid / 2u;
    let half    = tid % 2u;
    ...
    for (var block = block_group; block < num_blocks; block += num_block_groups) {
        ...
        for (var row = 0u; row < OUTPUTS_PER_WG; row++) {
            ...
            if (output_row < params.m) {
                ...
                // Read all 16 bytes of the sub-block (4 u32 words).
